...
export type ThumbnailType = 'empty' | 'image' | 'icon' | 'none';

export interface PopoverThumbnailProps {
  children: React.ReactNode;
  initialType?: ThumbnailType;
  thumbnailUrl?: string;
  onSave?: (type: ThumbnailType, imageUrl?: string) => void;
...
// Action button component for Upload, Canva, AI, Reset
...
// Tab type for internal UI (excludes 'empty' since it's not a visible tab)
type TabType = 'image' | 'icon' | 'none';

export default function PopoverThumbnail({
  children,
  initialType = 'empty',
  thumbnailUrl,
  onSave,
  ...
  // Map initialType to tab - 'empty' shows as 'image' tab
  const getTabFromType = (type: ThumbnailType): TabType => {
  ...
  // Persist last uploaded image so switching to "None" doesn't lose it locally
  const cachedImageRef = React.useRef<string | undefined>(thumbnailUrl);

  // Track if we have actual content (image or icon)
  const hasContent = !!imageUrl; // TODO: add icon check when icons are implemented

  // Reset showEditOptions when popover closes
  React.useEffect(() => {
    if (!open) {
  ...
  React.useEffect(() => {
    if (open) {
      ...
      // Edit options start closed - user must click pencil to open
      setShowEditOptions(false);
    }
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = event => {
        ...
        setShowEditOptions(false);
        // Immediately save when image is uploaded
        onSave?.('image', url);
        // Close popover after successful upload
        setOpen(false);
        onOpenChange?.(false);
      };
  ...
  const handleRemoveImage = () => {
    ...
    setShowEditOptions(false);
    // Removing image returns to empty state
    onSave?.('empty');
  };
  ...
  const overlay = open && (
    <div
      ...
      className={cn(
        'inset-0 z-40', // Removed backdrop-blur-[2px]
        overlayContainerRef?.current ? 'absolute' : 'fixed'
      )}
    />
  );

  // Determine which state to render for Image tab
        ...
        <div className="flex flex-col gap-4 px-5 pb-2 pt-5">
          ...
          <Tabs
            ...
            onValueChange={value => {
              ...
              // Only save type changes if there's actual content to show/hide
              if (hasContent) {
                if (newTab === 'none') {
                  // Preserve the uploaded image when hiding the thumbnail so the user can re-enable it
                  onSave?.('none', imageUrl);
                } else if (newTab === 'icon') {
                  onSave?.('icon');
                } else if (newTab === 'image') {
                  // Switching back to image tab with existing content
                  onSave?.('image', imageUrl);
                }
              }
              // If no content (empty), switching tabs doesn't save anything
            }}
          >
        ...
        >
          <div ref={measureRef} className="px-5 pb-5 pt-4">
            {selectedTab === 'image' && (
              <>
                {!hasImage ? (
                  // Empty State: Show three action buttons
                  ...
                  // Has Image State: Thumbnail with Edit/Delete buttons
                  <div className="flex flex-col gap-4">
                    ...
                    {/* Edit Options - animated show/hide */}
            ...
            {selectedTab === 'icon' && (
              <div className="flex items-center gap-4">
                <button
                  type="button"
                  className="flex size-16 flex-shrink-0 items-center justify-center overflow-hidden rounded-md bg-secondary"
            ...
            {selectedTab === 'none' && (
              <div className="flex items-center gap-4">
                <div className="flex size-16 flex-shrink-0 items-center justify-center rounded-md bg-secondary">
                  <EyeSlashIcon
                    size={24}
                    weight="regular"
                    className="text-primary"
